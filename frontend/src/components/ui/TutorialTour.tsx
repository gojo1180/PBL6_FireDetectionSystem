    ...
    <div className="fixed inset-0 z-[9990] overflow-hidden pointer-events-none">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="bg-white/90 backdrop-blur-xl border border-slate-100 shadow-[0_20px_50px_rgba(99,102,241,0.12)] rounded-2xl p-6 flex flex-col pointer-events-auto max-w-[90vw]"
        >
          {/* Header */}
          <div className="flex items-center justify-between mb-3.5">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 bg-slate-50 px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm">
            ...
            <div className="flex items-center gap-2">
              {/* Type Badge */}
              {stepType === "button" && (
                <span className="text-[9px] font-extrabold uppercase tracking-widest text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 flex items-center gap-1">
                  <Radio size={10} className="animate-pulse" />
                  Tombol
                </span>
              )}
              {stepType === "section" && (
                <span className="text-[9px] font-extrabold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 flex items-center gap-1">
                  <Box size={10} />
                  Seksi
                </span>
              )}
              {stepType === "center" && (
                <span className="text-[9px] font-extrabold uppercase tracking-widest text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
                  Info
                </span>
              )}
              <button
                onClick={onClose}
                className="p-1 rounded text-slate-400 hover:text-slate-600 hover:bg-slate-100/50 transition-colors ml-1"
                aria-label="Tutup"
              >
                <X size={14} />
              </button>
          ...
          {/* Title & Body */}
          <h4 className="text-sm font-extrabold text-slate-800 tracking-tight mb-1.5 flex items-center gap-1.5">
